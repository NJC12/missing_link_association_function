...
cat > ~/tmp/prepare.liu.uc.for.cojo.sh <<'EOF'
#!/bin/bash
...
/net/data/GTEx/gwas/liu_2015_and_goyette_2015_ibd/EUR.UC.gwas_info03_filtered.assoc \
> /net/data/GTEx/gwas/liu_2015_and_goyette_2015_ibd/cojo/ma_files/liu.uc.hg19.chr$chr.bed

/net/bin/x86_64/liftOver \
/net/data/GTEx/gwas/liu_2015_and_goyette_2015_ibd/cojo/ma_files/liu.uc.hg19.chr$chr.bed \
/net/home/nconnally/data/liftover_chains/hg19ToHg38.over.chain.gz \
/net/data/GTEx/gwas/liu_2015_and_goyette_2015_ibd/cojo/ma_files/liu.uc.hg38.chr$chr.bed \
/net/data/GTEx/gwas/liu_2015_and_goyette_2015_ibd/cojo/ma_files/liu.uc.unlifted.hg19.to.38.chr$chr.bed

awk -v chr=$chr '$1 == "chr"chr' \
/net/data/GTEx/gwas/liu_2015_and_goyette_2015_ibd/cojo/ma_files/liu.uc.hg38.chr$chr.bed | \
awk -F '[\t_]' 'BEGIN {print "SNP\tA1\tA2\tfreq\tb\tse\tp\tN"} {gsub(/chr/, ""); print $1 ":" $2+1 ":" $4 ":" $5 "\t" $4 "\t" $5 "\t" $6 "\t" $7 "\t" $8 "\t" $9 "\t" $10}' \
> /net/data/GTEx/gwas/liu_2015_and_goyette_2015_ibd/cojo/ma_files/liu.uc.hg38.chr$chr.ma

done
EOF

scp -o ProxyJump=nconnally@ika.bwh.harvard.edu \
~/tmp/prepare.liu.uc.for.cojo.sh \
nconnally@tako.partners.org:/net/data/GTEx/gwas/tmp_src

rm ~/tmp/prepare.liu.uc.for.cojo.sh

ssh -J nconnally@ika.bwh.harvard.edu \
nconnally@tako.partners.org \
'~/src/sgesh qsub /net/data/GTEx/gwas/tmp_src/prepare.liu.uc.for.cojo.sh'
...
cat > ~/tmp/run.cojo.liu.uc.chr$chr.sh <<EOF
#!/bin/bash
#$ -e $dir/eo_files
#$ -o $dir/eo_files
#$ -l mf=20G,h_vmem=20G
...
scp -o ProxyJump=nconnally@ika.bwh.harvard.edu \
~/tmp/run.cojo.liu.uc.chr*.sh \
nconnally@tako.partners.org:$dir/tmp_src

rm ~/tmp/run.cojo.liu.uc.chr*.sh

ssh -J nconnally@ika.bwh.harvard.edu \
nconnally@tako.partners.org \
"for chr in {1..22}; do
~/src/sgesh qsub $dir/tmp_src/run.cojo.liu.uc.chr\$chr.sh
done"
