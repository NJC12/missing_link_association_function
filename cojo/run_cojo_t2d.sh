...
### Create .ma reference file for COJO

cat > ~/tmp/prepare.mahajan.for.cojo.sh <<'EOF'
#!/bin/bash
#$ -e /net/data/GTEx/gwas/mahajan_2018_t2d/cojo/eo_files
#$ -o /net/data/GTEx/gwas/mahajan_2018_t2d/cojo/eo_files
#$ -l mf=5G,h_vmem=5G
#$ -p 0

for chr in {1..22}; do

awk -v chr=$chr 'NR > 1 && $2 == chr {print "chr" $2 "\t" $3-1 "\t" $3 "\t" $4 "_" $5 "_" $6 "_" $7 "_" $8 "_" $9 "_" $10;
print "chr" $2 "\t" $3-1 "\t" $3 "\t" $5 "_" $4 "_" $6 "_" $7 "_" $8 "_" $9 "_" $10}' \
/net/data/GTEx/gwas/mahajan_2018_t2d/Mahajan.NatGenet2018b.T2Dbmiadj.European.txt \
> /net/data/GTEx/gwas/mahajan_2018_t2d/cojo/ma_files/mahajan.hg19.chr$chr.bed

/net/bin/x86_64/liftOver \
/net/data/GTEx/gwas/mahajan_2018_t2d/cojo/ma_files/mahajan.hg19.chr$chr.bed \
/net/home/nconnally/data/liftover_chains/hg19ToHg38.over.chain.gz \
/net/data/GTEx/gwas/mahajan_2018_t2d/cojo/ma_files/mahajan.hg38.chr$chr.bed \
/net/data/GTEx/gwas/mahajan_2018_t2d/cojo/ma_files/mahajan.unlifted.hg19.to.38.chr$chr.bed

awk -v chr=$chr '$1 == "chr"chr' \
/net/data/GTEx/gwas/mahajan_2018_t2d/cojo/ma_files/mahajan.hg38.chr$chr.bed | \
awk -F '[\t_]' 'BEGIN {print "SNP\tA1\tA2\tfreq\tb\tse\tp\tN"} {gsub(/chr/, ""); print $1 ":" $2+1 ":" $4 ":" $5 "\t" $4 "\t" $5 "\t" $6 "\t" $7 "\t" $8 "\t" $9 "\t" $10}' \
> /net/data/GTEx/gwas/mahajan_2018_t2d/cojo/ma_files/mahajan.hg38.chr$chr.ma

done
EOF

scp -o ProxyJump=nconnally@ika.bwh.harvard.edu \
~/tmp/prepare.mahajan.for.cojo.sh \
nconnally@tako.partners.org:/net/data/GTEx/gwas/tmp_src

rm ~/tmp/prepare.mahajan.for.cojo.sh

ssh -J nconnally@ika.bwh.harvard.edu \
nconnally@tako.partners.org \
'~/src/sgesh qsub /net/data/GTEx/gwas/tmp_src/prepare.mahajan.for.cojo.sh'

### Running COJO

dir="/net/data/GTEx/gwas/mahajan_2018_t2d/cojo"
study="mahajan"
pheno="t2d"

for chr in {1..22}; do

cat > ~/tmp/run.cojo.mahajan.chr$chr.sh <<EOF
#!/bin/bash
#$ -e $dir/eo_files
#$ -o $dir/eo_files
#$ -l mf=20G,h_vmem=20G
...
scp -o ProxyJump=nconnally@ika.bwh.harvard.edu \
~/tmp/run.cojo.mahajan.chr*.sh \
nconnally@tako.partners.org:$dir/tmp_src

rm ~/tmp/run.cojo.mahajan.chr*.sh

ssh -J nconnally@ika.bwh.harvard.edu \
nconnally@tako.partners.org \
"for chr in {1..22}; do
~/src/sgesh qsub $dir/tmp_src/run.cojo.mahajan.chr\$chr.sh
done"
...
cat > ~/tmp/lift.mahajan.to.hg19.sh <<EOF
#!/bin/bash
#$ -e $dir/eo_files
#$ -o $dir/eo_files
#$ -l mf=20G,h_vmem=20G
...
awk -F '[\t:]' '{print "chr"\$1 "\t" \$6-1 "\t" \$6 "\t" \$4 "_" \$5 "_" \$7 "_" \$8 "_" \$9 "_" \$10 "_" \$11 "_" \$12 "_" \$13 "_" \$14 "_" \$15 "_" \$16}' \
...
$dir/results_hg19/$study.$pheno.unlifted.hg38.to.19.bed
...
scp -o ProxyJump=nconnally@ika.bwh.harvard.edu \
~/tmp/lift.mahajan.to.hg19.sh \
nconnally@tako.partners.org:$dir/tmp_src

rm ~/tmp/lift.mahajan.to.hg19.sh

ssh -J nconnally@ika.bwh.harvard.edu \
nconnally@tako.partners.org \
"~/src/sgesh qsub $dir/tmp_src/lift.mahajan.to.hg19.sh"
